...
void Solver::set_decision_priority_seed (uint32_t seed) {
  LOG_API_CALL_BEGIN ("set_decision_priority_seed");
  REQUIRE_VALID_STATE ();
  internal->set_decision_priority_seed (seed);
  LOG_API_CALL_END ("set_decision_priority_seed");
}

void Solver::clear_decision_priority () {
  LOG_API_CALL_BEGIN ("clear_decision_priority");
  REQUIRE_VALID_STATE ();
  internal->clear_decision_priority ();
  LOG_API_CALL_END ("clear_decision_priority");
}

void Solver::add_decision_priority_lit (int lit, uint32_t priority) {
  LOG_API_CALL_BEGIN ("add_decision_priority_lit");
  REQUIRE_VALID_STATE ();
  REQUIRE_VALID_LIT (lit);
  const int ilit = external->internalize (lit);
  internal->add_decision_priority_lit (ilit, priority);
  LOG_API_CALL_END ("add_decision_priority_lit");
}

NativeDecisionPriorityStats Solver::decision_priority_stats () const {
  return internal->decision_priority_stats;
}
