...
struct NativeDecisionPriorityStats {
  uint64_t add_lit_calls = 0;
  uint64_t add_lit_duplicates = 0;
  uint64_t unique_lits = 0;
  uint64_t max_priority_buckets = 0;
  uint64_t seed = 0;
  uint64_t decide_calls = 0;
  uint64_t decide_returns = 0;
  uint64_t decide_fallbacks = 0;
  uint64_t decide_positive = 0;
  uint64_t decide_negative = 0;
};
...
public:
  ...
  // Native decision-priority hints. Lower priority values are selected
  // before higher priority values. The sign of 'lit' is ignored for the
  // decision phase; priority decisions use a seed-derived phase instead.
  //
  //   require (VALID)
  //   ensure (VALID)
  //
  ...
  NativeDecisionPriorityStats decision_priority_stats () const;
