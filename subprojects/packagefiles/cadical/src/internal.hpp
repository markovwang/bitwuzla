...
struct DecisionPriorityCandidate {
  uint32_t priority = 0;
  uint64_t key = 0;
  int idx = 0;
  uint64_t generation = 0;

  bool operator< (const DecisionPriorityCandidate &other) const {
...
struct Internal {
  ...
  uint32_t decision_priority_seed; // Seed for native decision priorities.
  uint64_t decision_priority_generation; // Monotonic candidate generation.
  NativeDecisionPriorityStats decision_priority_stats;
  vector<DecisionPriorityEntry> decision_priority_entries;
  set<DecisionPriorityCandidate> decision_priority_candidates;
  map<uint32_t, uint64_t> decision_priority_bucket_sizes;
  ...
  //
  void set_decision_priority_seed (uint32_t seed);
  void clear_decision_priority ();
  void add_decision_priority_lit (int lit, uint32_t priority);
  DecisionPriorityCandidate decision_priority_candidate (int idx) const;
  void enqueue_decision_priority (int idx);
  void dequeue_decision_priority (int idx);
  void refresh_decision_priority (int idx);
  int next_decision_priority_variable ();
  int decision_priority_phase (int idx);
  bool satisfied ();
